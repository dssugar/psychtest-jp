...
import { sccsQuestions, scaleLabels } from "@/data/sccs-questions";
import { calculateSccsScore, validateAnswerPattern } from "@/lib/scoring/sccs";
import { saveTestResult } from "@/lib/storage";
import { DataBadge } from "@/components/viz/DataBadge";

export default function SccsTestPage() {
  const router = useRouter();
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [answers, setAnswers] = useState<number[]>(new Array(12).fill(0));
  const [showWarning, setShowWarning] = useState(false);

  const progress = ((currentQuestion + 1) / sccsQuestions.length) * 100;
  ...
  const handleAnswer = (value: number) => {
    ...
    // 次の質問へ自動的に進む
    if (currentQuestion < sccsQuestions.length - 1) {
      setTimeout(() => {
  ...
  const handleSubmit = () => {
    ...
    // スコア計算
    const result = calculateSccsScore(answers);

    // 結果を保存（新API使用）
    saveTestResult("sccs", result, answers);

    // 結果ページへ遷移
    router.push("/results/sccs");
  };

  const question = sccsQuestions[currentQuestion];
  const isLastQuestion = currentQuestion === sccsQuestions.length - 1;
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-4">
                <div className="font-mono font-bold text-sm text-brutal-gray-800">
                  質問 {currentQuestion + 1} / {sccsQuestions.length}
                </div>
                <div className="font-mono font-bold text-sm text-brutal-gray-800">
                  {Math.round(progress)}%
                </div>
