...
/**
 * 後方互換性のための関数（summary のみ返す）
 * @deprecated getDetailedInterpretation() を使用してください
 */
export function getInterpretation(level: SwlsResult["level"]): string {
  const detailed = getDetailedInterpretation(level);
...
function getVeryLowInterpretation(): InterpretationData {
  return {
    summary: `人生に対して極めて強い不満を感じている状態です（スコア: 5-9点）。現在の生活のほとんどの側面において満足を得られておらず、理想と現実の大きなギャップに苦しんでいる可能性があります。日常生活に支障をきたしている場合もあり、専門家のサポートを検討すべきレベルです。`,

    dailyLifeImpact: `人生全体への不満が対人関係にも影響し、他者との交流に消極的になったり、孤立感を感じたりすることがあります。「自分の人生はうまくいっていない」という思いから、他者と自分を比較し、劣等感を抱きやすくなります。親しい人に対しても悲観的な態度を示し、関係性に緊張が生じることがあります。
...
慢性的なストレスにより、睡眠障害、食欲不振、疲労感などの身体症状が現れることがあります。うつ病や不安障害を併発している可能性も高く、注意が必要です。`,
    ...
    psychBackground: `研究によると、SWLSスコアが極めて低い人は、うつ病（PHQ-9）や不安障害（GAD-7）との強い負の相関を示します（r = -0.40 to -0.50）。また、自尊心の低さ、神経症傾向の高さとも関連しています。
...
function getLowInterpretation(): InterpretationData {
  return {
    summary: `人生に対して不満を感じている状態です（スコア: 10-14点）。現在の生活に満足できておらず、多くの領域で改善が必要だと感じています。日常生活は送れていますが、慢性的な不満やストレスを抱えており、生活の質（QOL）が低下しています。積極的な対処が必要なレベルです。`,

    dailyLifeImpact: `対人関係に満足していないことが多く、孤独感や疎外感を感じることがあります。他者との交流はありますが、深いつながりや親密さを感じにくい状態です。「誰も自分を理解してくれない」という思いから、表面的な付き合いに留まりがちです。
...
慢性的なストレスにより、疲労感、頭痛、肩こり、胃腸の不調などの身体症状が現れることがあります。睡眠の質が低下し、朝起きても疲れが取れていないことが多いです。`,
    ...
    psychBackground: `SWLSスコアが10-14点の範囲にある人は、米国の基準では「明らかに平均を下回る」レベルですが、日本の平均（18.9点）と比較しても低い状態です。
...
function getSlightlyBelowInterpretation(): InterpretationData {
  return {
    summary: `人生にやや不満を感じている状態です（スコア: 15-19点）。日本の平均（18.9点）に近く、多くの人が経験するレベルです。現在の生活に部分的に満足していますが、改善したい領域もあります。大きな問題はありませんが、より充実した人生を送るためには積極的な取り組みが有効です。`,

    dailyLifeImpact: `対人関係は概ね良好ですが、親密さや深いつながりに欠けると感じることがあります。表面的な付き合いは多いものの、「本当に理解し合える友人」が少ないと感じるかもしれません。孤独感を時折感じますが、社会的孤立というほどではありません。
...
健康状態は概ね良好ですが、ストレスが溜まると疲労感や軽い不調（頭痛、胃もたれ）を感じます。運動不足や不規則な生活習慣が気になっています。`,
    ...
    psychBackground: `SWLSスコア15-19点は、日本人の平均的な範囲に含まれます（平均18.9点）。米国の基準では「やや平均を下回る」とされますが、文化差を考慮すると、日本では「普通」と言えます。
...
function getNeutralInterpretation(): InterpretationData {
  return {
    summary: `人生の満足度が平均的な状態です（スコア: 20-24点）。日本人の平均（18.9点）よりやや高く、米国では「やや平均を上回る」レベルです。現在の生活に概ね満足していますが、さらに充実させる余地もあります。安定した状態ですが、意識的な努力により、より高い幸福度を目指すことができます。`,

    dailyLifeImpact: `対人関係は良好で、家族や友人とのつながりを大切にしています。深い関係性もいくつかあり、必要な時にはサポートを得られます。ただし、新しい出会いや関係の深化にはやや消極的な面もあります。
...
心身ともに健康で、ストレス対処能力も適切に機能しています。定期的な運動や趣味の時間を確保し、ワークライフバランスを保てています。睡眠の質も良好で、日中のエネルギーレベルも安定しています。`,
    ...
    psychBackground: `SWLSスコア20-24点は、幸福研究において「安定した幸福」のベースラインと見なされます。このレベルの人は、以下の特徴を持つことが研究で示されています：
...
function getSlightlyAboveInterpretation(): InterpretationData {
  return {
    summary: `人生にやや満足している状態です（スコア: 25-29点）。米国の基準では「平均を上回る」、日本と比較すると「かなり高い」満足度です。現在の生活に概ね満足しており、ポジティブな感情が優勢です。充実した人間関係、やりがいのある仕事、良好な健康状態を維持しています。さらに幸福度を高める余地はありますが、既に高いウェルビーイングの状態です。`,

    dailyLifeImpact: `深く充実した人間関係を持ち、家族や友人との強い絆を感じています。相互にサポートし合える関係性があり、孤独感をほとんど感じません。他者との交流を楽しみ、新しい出会いにも積極的です。共感力が高く、他者の幸福にも関心を持っています。
...
心身ともに健康で、エネルギーレベルが高い状態です。ストレス対処能力が高く、困難な状況でもレジリエンス（回復力）を発揮できます。定期的な運動、質の高い睡眠、バランスの取れた食事を維持し、セルフケアを大切にしています。`,
    ...
    psychBackground: `SWLSスコア25-29点は、幸福研究において「高い幸福度」と分類されます。このレベルの人は、ポジティブ心理学で「フラリッシング（flourishing）」と呼ばれる状態に近く、以下の特徴を持ちます：
...
**健康への好影響**: 高い人生満足度は、身体的健康（免疫機能、心血管系）、寿命の延伸と正の相関があることが複数の縦断研究で示されています。`,
    ...
    practicalAdvice: `**現在の幸福を維持・深化させる**
...
function getHighInterpretation(): InterpretationData {
  return {
    summary: `人生に満足している状態です（スコア: 30-34点）。米国でも「平均を大きく上回る」、日本では「極めて高い」満足度です。人生のほとんどの領域において充実を感じており、強い幸福感を持っています。困難があっても前向きに対処でき、総合的に人生を肯定的に捉えています。この状態を維持し、さらに深めることが目標となります。`,

    dailyLifeImpact: `非常に充実した人間関係を持ち、愛情、親密さ、相互サポートを日常的に経験しています。家族、友人、コミュニティとの強い絆があり、孤独を感じることはほとんどありません。他者への共感と思いやりが深く、人間関係に喜びと意味を見出しています。新しい出会いにも開かれており、人脈が自然と広がります。
...
心身ともに極めて健康で、高いエネルギーと活力を持っています。ストレスレジリエンスが非常に高く、困難な状況でも冷静に対処できます。セルフケアが生活に統合されており、意識せずとも健康的な習慣を維持しています。`,
    ...
    psychBackground: `SWLSスコア30-34点は、幸福研究において「非常に高い幸福度」と分類され、ポジティブ心理学の「フラリッシング（flourishing）」の典型例です。
...
function getVeryHighInterpretation(): InterpretationData {
  return {
    summary: `人生に対して極めて高い満足を感じている状態です（スコア: 35点/35点満点）。人生のすべての項目で最高評価をつけており、理想と現実が一致している稀有な状態です。深い幸福感、充実感、人生の意味を感じています。ただし、完璧主義や天井効果の可能性もあり、この状態の意味を深く理解することが重要です。`,

    dailyLifeImpact: `極めて深く充実した人間関係を持ち、愛情、信頼、相互理解に満ちた絆を日常的に経験しています。他者との交流が人生の大きな喜びの源となっており、与えることと受け取ることのバランスが完璧に取れています。コミュニティの中心的存在として、多くの人に前向きな影響を与えています。
...
心身ともに最高の状態で、生命力にあふれています。ストレスをエネルギーに変える能力があり、困難を成長の機会と捉えます。セルフケアが自然と統合されており、健康的なライフスタイルが習慣化しています。`,
    ...
    psychBackground: `SWLSで満点（35点）を取ることは統計的に稀で、一般集団では約1-2%程度と推定されます。このスコアは以下の可能性を示唆します：
